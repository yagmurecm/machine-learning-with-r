...
##Veri 
```{r} 
df <- df %>% dplyr::select(-c("Hastane_Adi","Bildirim_İlcesi","Mernis_il","Mernis_ilce","Mernis_mahalle"))

df2 <- df[c("Yas" ,  "CRP" , "Oksijen" , "Vaka_Durumu" , "Entübasyon_Var_mi" ,"Ferritin", "Yogun_Bakimda_Mi")]
df3 <- df[c("Yas" ,  "CRP" , "Oksijen" , "Vaka_Durumu" , "Entübasyon_Var_mi" )]
df4 <- df[c("Cinsiyet","Hasta_Takip_Durumu", "Vaka_Durumu","Temaslı_mi","Yogun_Bakimda_Mi","Entübasyon_Var_mi","Yas","BT_Sonucu","BT","Pnomoni", "Sürec_Durumu","Ferritin","CRP","Oksijen")]
...
model5 <- lm(CRP ~  Vaka_Durumu +Entübasyon_Var_mi + BT_Sonucu + BT + Sürec_Durumu + Oksijen, data  = trainSet5)
summary(model5)
...
model6 <- lm(CRP ~ Vaka_Durumu +Entübasyon_Var_mi + BT_Sonucu + BT + Sürec_Durumu + Oksijen , data  = trainSetRemovedOut)
summary(model6)
summary(model5)
...
modelData <- df %>% dplyr::select(Hastane_Adi , Cinsiyet , Hasta_Takip_Durumu , Vaka_Durumu , Temaslı_mi , Gebe_Mi , Bildirim_İlcesi , Yogun_Bakimda_Mi , Entübasyon_Var_mi , Yas , BT_Sonucu , BT , Pnomoni , Sürec_Durumu , Ferritin , CRP , Oksijen)


num_cols <- c("CRP" , "Ferritin", "Oksijen", "Yas")
...
fitGl <- glmnet(trainSet_x , trainSet_y  , alpha = 0 , lambda = 0.01122)
...
##Model
```{r}
modelData<-df %>% dplyr::select(-c("Hastane_Adi","Bildirim_İlcesi","Mernis_il","Mernis_ilce","Mernis_mahalle"))
...
predictModel <- glmnet(trainDataX , trainDataY , alpha = 0.9157278, lambda = 0.01188862 )
...
df<- df %>% dplyr::select(c("Cinsiyet","Hasta_Takip_Durumu","Vaka_Durumu","Temaslı_mi","Yogun_Bakimda_Mi","Entübasyon_Var_mi","Yas","BT_Sonucu","BT","CRP","Pnomoni","Ferritin","Oksijen"))
...
##model
```{r}
pcr_fit <- pcr(CRP~ Cinsiyet+Hasta_Takip_Durumu+Vaka_Durumu+Temaslı_mi+Yogun_Bakimda_Mi+Entübasyon_Var_mi+Yas+BT_Sonucu+BT+Pnomoni+Ferritin+Oksijen, data = training,    scale = TRUE,    validation = "CV")
...
pred = as.vector(predict(pcr_tune, train_x)))
)
...
df %>% mutate(y_sapka = predict(cart_tree)) %>%
  ggplot() +
  geom_point(aes(Oksijen, CRP)) +
  geom_step(aes(Oksijen, y_sapka), col = "red")
...
defaultSummary(data.frame(obs = df$CRP, pred = predict(prune_cart)))
...
plot(predict(cart_tune), df$CRP, xlab = "Tahmin",ylab = "Gercek")
abline(0,1)
...
defaultSummary(data.frame(obs = df$CRP,
...
#ikinci yöntem
bag_fit <- bagging(CRP~ ., data = training)

#üçüncü yöntem
bag_fit <- randomForest(training$CRP ~ . , data = training,
...
names(df) <- c("Hospital","Sex","Patient_Followup_Status", "Case_Status","","Pregnancy","Notification_District","Intensive_care","Intubation","Age","CT_Result","CT","Pneumonia", "Process_Status", "Province","District","Neighborhood","Ferritin","CRP","Oxygen")
# Model 2 (Ferritinsiz)
...
caret::confusionMatrix(predModelBase_train , train$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel2_train , train$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel3_train , train$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel4_train , train$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
...
caret::confusionMatrix(predModelBase , test$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel2 , test$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel3 , test$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
caret::confusionMatrix(predModel4 , test$Vaka_Durumu , mode = "prec_recall", positive="Kötü")
...
modelTuned <- multinom(Vaka_Durumu ~ . , 
                       data  = train ,
                       decay = 0	
                      )
...
caret::confusionMatrix(predModelTuned , test$Vaka_Durumu , mode = "prec_recall")
...
set.seed(123)
train_indeks <- createDataPartition(df$Vaka_Durumu, p = 0.8, list = FALSE, times = 1)
...
knn_train <- knn_train %>% dplyr::select(-c("Hastane_Adi","Bildirim_İlcesi","BT_Sonucu", "Sürec_Durumu", "Mernis_il","Mernis_ilce","Mernis_mahalle"))
knn_test <- knn_test %>% dplyr::select(-c("Hastane_Adi","Bildirim_İlcesi","BT_Sonucu", "Sürec_Durumu", "Mernis_il","Mernis_ilce","Mernis_mahalle"))


knn_train <- knn_train %>% dplyr:: select(-Vaka_Durumu)
knn_test <- knn_test %>% dplyr:: select(-Vaka_Durumu)
...
cart_tree <- tree(Vaka_Durumu~., data = train)
...
```{r}
case_rpart <- rpart(Vaka_Durumu ~ ., data = train, method = "class")
...
prp(case_rpart_prune, type = 1)
rpart.plot(case_rpart_prune)
...
```{r}
case_tree <- tree(Vaka_Durumu~ . , data = train)
...
rf_gridsearch <- train(Vaka_Durumu ~ ., 
                       data = train,
                       method = 'rf',
                       metric = 'Accuracy',
                       tuneGrid = tunegrid)
...
confusionMatrix(predict(rf_random, test_x), test_y, positive = "1")
...
train$Vaka_Durumu<- as.numeric(train$Vaka_Durumu)
train <- transform(train, Vaka_Durumu = Vaka_Durumu - 1)
df$`Vaka_Durumu`<-as.factor(df$`Vaka_Durumu`)

gbm_fit <- gbm(Vaka_Durumu~., data = train, 
               shrinkage = 0.01,
